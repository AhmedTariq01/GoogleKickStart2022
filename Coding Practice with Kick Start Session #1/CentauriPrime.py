...
# Before they start playing the game, they have to choose rulers for each kingdom. Alice and Bob mutually decided to distribute kingdoms based
# on the letter the kingdom's name ended with. Alice loves vowels and decided to rule kingdoms whose names ended in a vowel.
# Bob loves consonants and decided to rule kingdoms whose names ended in a consonant. Both of them do not like the letter 'y'(case insensitive) and thus,
# all kingdoms whose names ended in the letter 'y' are left without a ruler. Can you write a program that will determine the rulers of several kingdoms,
# given the kingdoms names?
# Input
...
number_of_testcases = int(input("Enter number of test cases: "))
cases = 0
while cases < number_of_testcases:
    cases += 1
    text = str(input("Enter text: "))
    if text[-1] == "y" or text[-1] == "Y":
        print("Case #{}: {} is ruled by nobody.".format(cases, text))
    elif text[-1] in ['A', 'E', 'I', 'O', 'U', 'a', 'e', 'i', 'o', 'u']:
        print("Case #{}: {} is ruled by Alice.".format(cases, text))
    else:
        print("Case #{}: {} is ruled by Bob.".format(cases, text))
