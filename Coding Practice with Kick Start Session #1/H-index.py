...
# Jorge has written N papers in his lifetime. The i-th paper has Ci citations. Each paper was written sequentially in the order provided, and the
# number of citations that each paper has will never change. Please help Jorge determine his H-index score after each paper he wrote.
# Input
# The first line of the input gives the number of test cases, T. T test cases follow. Each test case begins with a line containing N, the number
# of papers Jorge wrote. The second line contains N integers. The i-th integer is Ci, the number of citations that the i-th paper has.
# Output
...
for t in range(number_of_testCases):
    papers_code = int(input("Enter the number of papers: "))
    # List C : citations for each paper
    citations = list(map(int, input("Enter the number of citations: ").strip().split()))

    print(f"Case #{t + 1}:", end=" ")

    # calculating and displaying h index uptil each paper
    for n in range(1, papers_code + 1):
        count = [0] * (n + 2)
        for i in range(n):
            if citations[i] > n:
                count[n + 1] += 1
